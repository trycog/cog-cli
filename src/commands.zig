...
pub fn init(allocator: std.mem.Allocator, args: []const [:0]const u8) !void {
    ...
    // If the background check already knows about a newer release, mention it
    // once, dimly. Cache-only: init never polls the network for this.
    if (update_check_mod.pendingNoticeFromCache(allocator)) |notice| {
        defer notice.deinit(allocator);
        var notice_buf: [192]u8 = undefined;
        if (std.fmt.bufPrint(&notice_buf, "\n  " ++ dim ++ "Cog v{s} is available (installed v{s}) — brew upgrade cog" ++ reset ++ "\n", .{ notice.latest, notice.installed }) catch null) |msg| {
            printErr(msg);
        }
        debug_log.log("init: cached update notice shown latest={s}", .{notice.latest});
    }
}
...
fn readAssetVersionStamp(allocator: std.mem.Allocator, cog_dir: []const u8) ?[]u8 {
    const ctx_path = std.fmt.allocPrint(allocator, "{s}/client-context.json", .{cog_dir}) catch return null;
    defer allocator.free(ctx_path);

    const file = std.fs.openFileAbsolute(ctx_path, .{}) catch |err| {
        debug_log.log("readAssetVersionStamp: no client-context.json: {s}", .{@errorName(err)});
        return null;
    };
    const content = file.readToEndAlloc(allocator, 1048576) catch {
        file.close();
        debug_log.log("readAssetVersionStamp: client-context.json unreadable", .{});
        return null;
    };
    file.close();
    defer allocator.free(content);
    ...
    const parsed = std.json.parseFromSlice(std.json.Value, allocator, content, .{}) catch {
        debug_log.log("readAssetVersionStamp: client-context.json malformed", .{});
        return null;
    };
    defer parsed.deinit();
...
pub fn doctor(allocator: std.mem.Allocator, args: []const [:0]const u8) !void {
    ...
    printErr("\n" ++ cyan ++ bold ++ "  Updates" ++ reset ++ "\n");

    switch (update_check_mod.statusForDoctor(allocator)) {
        .disabled => {
            printErr("    " ++ dim ++ "- Update check disabled (" ++ update_check_mod.OPT_OUT_ENV ++ "=0)" ++ reset ++ "\n");
            debug_log.log("doctor: update check disabled by env", .{});
        },
        .unknown => {
            printErr("    " ++ dim ++ "- Update status unknown" ++ reset ++ "\n");
            debug_log.log("doctor: update status unknown", .{});
        },
        .up_to_date => {
            var up_buf: [128]u8 = undefined;
            const up_msg = std.fmt.bufPrint(&up_buf, "    " ++ cyan ++ check ++ reset ++ " Cog is up to date (v{s})\n", .{update_check_mod.installed_version}) catch
                "    " ++ cyan ++ check ++ reset ++ " Cog is up to date\n";
            printErr(up_msg);
            passed += 1;
            debug_log.log("doctor: cog is up to date", .{});
        },
        .update_available => |notice| {
            defer notice.deinit(allocator);
            var avail_buf: [192]u8 = undefined;
            const avail_msg = std.fmt.bufPrint(&avail_buf, "    " ++ yellow ++ "!" ++ reset ++ " Update available: v{s} (installed v{s}) — brew upgrade cog\n", .{ notice.latest, notice.installed }) catch
                "    " ++ yellow ++ "!" ++ reset ++ " Update available — brew upgrade cog\n";
            printErr(avail_msg);
            warnings += 1;
            debug_log.log("doctor: update available latest={s}", .{notice.latest});
        },
    }

    // Asset drift: were the installed agent assets generated by this binary?
    // Report only — regeneration stays the user's decision via cog init.
    asset_drift: {
        const stamp: ?[]u8 = if (maybe_cog_dir) |cd| readAssetVersionStamp(allocator, cd) else null;
        defer if (stamp) |s| allocator.free(s);

        const asset_version = stamp orelse {
            printErr("    " ++ yellow ++ "!" ++ reset ++ " installed agent assets predate version stamping — run cog init to refresh\n");
            warnings += 1;
            debug_log.log("doctor: no cog_version stamp in client-context.json", .{});
            break :asset_drift;
        };

        if (std.mem.eql(u8, asset_version, update_check_mod.installed_version)) {
            var cur_buf: [128]u8 = undefined;
            const cur_msg = std.fmt.bufPrint(&cur_buf, "    " ++ cyan ++ check ++ reset ++ " Agent assets current (v{s})\n", .{asset_version}) catch
                "    " ++ cyan ++ check ++ reset ++ " Agent assets current\n";
            printErr(cur_msg);
            passed += 1;
            debug_log.log("doctor: agent assets current", .{});
        } else {
            var drift_asset_buf: [224]u8 = undefined;
            const drift_asset_msg = std.fmt.bufPrint(&drift_asset_buf, "    " ++ yellow ++ "!" ++ reset ++ " agent assets generated by v{s}, binary is v{s} — run cog init to refresh\n", .{ asset_version, update_check_mod.installed_version }) catch
                "    " ++ yellow ++ "!" ++ reset ++ " agent assets predate this binary — run cog init to refresh\n";
            printErr(drift_asset_msg);
            warnings += 1;
            debug_log.log("doctor: asset drift assets={s} binary={s}", .{ asset_version, update_check_mod.installed_version });
        }
    }
...
test "readAssetVersionStamp tolerates missing files and fields" {
    try withTempCwd(struct {
        fn run(allocator: std.mem.Allocator) !void {
            try std.fs.cwd().makeDir(".cog");
            ...
            // Missing file: assets predate stamping, never an error.
            try std.testing.expect(readAssetVersionStamp(allocator, cog_dir) == null);

            // Field absent (written by an older binary): still null.
            try std.fs.cwd().writeFile(.{ .sub_path = ".cog/client-context.json", .data = "{\n  \"selected_agents\": []\n}\n" });
            try std.testing.expect(readAssetVersionStamp(allocator, cog_dir) == null);

            // Malformed JSON: tolerated.
            try std.fs.cwd().writeFile(.{ .sub_path = ".cog/client-context.json", .data = "{not json" });
            try std.testing.expect(readAssetVersionStamp(allocator, cog_dir) == null);
