...
fn printDebugHelp() void {
    ...
    printErr(
        ...
        ++ "    " ++ dim ++ "... and 34 more (debug/send_* commands)" ++ reset ++ "\n"
        ++ "\n"
        ++ dim ++ "  Run 'cog debug/<command> --help' for details on a specific command.\n"
        ++ "  CLI tools auto-start the daemon and return clean JSON." ++ reset ++ "\n"
        ++ "\n"
    );
}
