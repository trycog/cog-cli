...
fn openrouterPost(allocator: std.mem.Allocator, api_key: []const u8, body: []const u8) ![]const u8 {
    ...
    const response = http.post(allocator, OPENROUTER_URL, &headers, body) catch {
    ...
    if (response.status_code != 200) {
        ...
        const msg = std.fmt.bufPrint(&msg_buf, "  error: HTTP status {d}\n", .{response.status_code}) catch "  error: HTTP error\n";
        printErr(msg);
        if (json.parseFromSlice(json.Value, allocator, response.body, .{})) |p| {
            defer p.deinit();
            if (p.value == .object) {
