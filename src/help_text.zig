...
pub const debug_serve =
    ...
    ++ "\n"
    ++ "  Start an MCP debug server over stdio. Exposes debug tools\n"
    ++ "  (launch, breakpoint, run, inspect, stop) that AI agents can\n"
    ++ "  use to step-debug programs.\n"
    ++ "\n"
    ++ cyan ++ bold ++ "  Usage" ++ reset ++ "\n"
    ++ "    cog debug/serve\n"
    ++ "\n"
    ++ cyan ++ bold ++ "  Tools" ++ reset ++ "\n"
    ++ "    " ++ bold ++ "debug_launch" ++ reset ++ "          " ++ dim ++ "Launch a program under the debugger" ++ reset ++ "\n"
    ++ "    " ++ bold ++ "debug_breakpoint" ++ reset ++ "      " ++ dim ++ "Set, remove, or list breakpoints" ++ reset ++ "\n"
    ++ "    " ++ bold ++ "debug_run" ++ reset ++ "             " ++ dim ++ "Continue, step, or restart execution" ++ reset ++ "\n"
    ++ "    " ++ bold ++ "debug_inspect" ++ reset ++ "         " ++ dim ++ "Evaluate expressions and inspect variables" ++ reset ++ "\n"
    ++ "    " ++ bold ++ "debug_stop" ++ reset ++ "            " ++ dim ++ "Stop a debug session" ++ reset ++ "\n"
    ++ "\n"
