# Cog Debug Server — End-to-End Test Prompt (TypeScript / DAP)
...
```json
{"name":"debug_launch","arguments":{"program":"/tmp/ts/debug_test.js","stop_on_entry":true}}
```

```json
{"name":"debug_breakpoint","arguments":{"session_id":"session-1","action":"set","file":"/tmp/ts/debug_test.ts","line":4}}
```
...
### DAP Backend Notes (TypeScript)

TypeScript debugging uses the **DAP (Debug Adapter Protocol)** transport via `vscode-js-debug`
(`dapDebugServer.js`). The DAP server is auto-downloaded to `~/.config/cog/js-debug/` on first use
and communicates over TCP on a dynamically assigned port. TypeScript uses the **same DAP adapter**
as JavaScript (`pwa-node`), but adds a **source map layer**:

- TypeScript programs are **compiled** to JavaScript with `tsc --sourceMap`.
- The compiled `.js` file is **launched** as the `program` in `debug_launch`.
- **Breakpoints are set on the `.ts` source files** — the DAP adapter resolves them
  to the correct `.js` positions via source maps (`.js.map` files).
- This tests **source map debugging** — a key capability not exercised by the JavaScript E2E.
...
Before running any scenarios, compile all TypeScript test fixtures:

```bash
bash prompts/fixtures/ts/setup.sh
```

This copies `.ts` source files to `/tmp/ts/`, then compiles them with `tsc --sourceMap` to
produce `.js` and `.js.map` files alongside the sources. The compiled `.js` files are launched
by the debugger; the `.ts` files are used for breakpoint locations via source maps. Run the
...
**Do NOT use sleep-based programs in this E2E.** Never launch `/tmp/ts/debug_sleep.js`, never run `/usr/bin/sleep`, and never add waits via sleep commands.

### Programs

| Program | Source | Compiled (launched) | Used by |
|---------|--------|---------------------|---------|
| A: Basic | `/tmp/ts/debug_test.ts` | `/tmp/ts/debug_test.js` | Scenarios 1-11, 14-18, 20-28, 30-33 |
| B: Crasher | `/tmp/ts/debug_crash.ts` | `/tmp/ts/debug_crash.js` | Scenario 19 |
| D: Multi-variable | `/tmp/ts/debug_vars.ts` | `/tmp/ts/debug_vars.js` | Scenarios 12-13, 29 |

### Key Line References

**IMPORTANT:** The `program` parameter in `debug_launch` always uses the compiled **`.js`** file.
The `file` parameter in `debug_breakpoint` (and other source-referencing tools) always uses the
**`.ts`** source file. Source maps handle the mapping.

**Program A** (`debug_test.ts`):
...
**Program D** (`debug_vars.ts`):
- Line 23: `const local3 = local1 * local2;` (inside `processData()`)
- Line 32: `x = modify(x, 3);` (inside `main()`)
...
This is the baseline test. Verify the fundamental debug loop works with source-mapped TypeScript.

1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `session_id`, `file: "/tmp/ts/debug_test.ts"`, `line: 4`
...
1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 36 (`const result1 = compute(x, y);` in `main()`)
...
scopes change correctly when stepping into/out of functions. Source maps must correctly resolve
step targets across function boundaries.
...
1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 4 (inside `add()`)
3. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 9 (inside `multiply()`)
4. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 16 (`const final_ = sum + product;` in `compute()`)
...
**What this tests:** The debugger correctly arms multiple breakpoints (via source maps) and hits
them in execution order. Each continue resumes and stops at the next breakpoint.
...
1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 23 (`total = add(total, i);` in `loopSum()`)
...
1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 14 (`const sum = add(x, y);` in `compute()`)
...
1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 4 — note the returned breakpoint ID
3. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 9 — note the returned breakpoint ID
4. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 34 — note the returned breakpoint ID
...
1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 29 (`if (n <= 1) return 1;` in `factorial()`)
...
1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 34 (`const x = 10;` in `main()`)
...
Uses Program D (`/tmp/ts/debug_vars.js`).

1. `debug_launch` with `program: "/tmp/ts/debug_vars.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_vars.ts"` at line 23 (`const local3 = local1 * local2;` inside `processData()`)
3. `debug_run` with `action: "continue"` to hit the breakpoint
...
1. `debug_launch` with `program: "/tmp/ts/debug_vars.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_vars.ts"` at line 32 (`x = modify(x, 3);` in `main()`)
3. `debug_run` with `action: "continue"` to hit the breakpoint
...
when used with the DAP/vscode-js-debug backend. TypeScript debugging runs through DAP,
...
4. `debug_breakpoint_locations` with `source: "/tmp/ts/debug_test.ts"`, `line: 3`, `end_line: 6`
   — should return valid breakpoint positions within `add()` (lines 4 and 5 at minimum)
5. `debug_breakpoint_locations` with `source: "/tmp/ts/debug_test.ts"`, `line: 12`
...
Uses Program B (`/tmp/ts/debug_crash.js`).

1. `debug_launch` with `program: "/tmp/ts/debug_crash.js"`, `args: ["null"]`, `stop_on_entry: true`
...
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 14 (`const sum = add(x, y);` in `compute()`)
3. `debug_run` with `action: "continue"` to hit line 14
4. `debug_goto_targets` with `file: "/tmp/ts/debug_test.ts"`, `line: 16`
   — should return error with code `-32001` (NOT_SUPPORTED)
5. `debug_run` with `action: "goto"`, `file: "/tmp/ts/debug_test.ts"`, `line: 16`
   — should return error with code `-32001` (NOT_SUPPORTED)
...
1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 14 (`const sum = add(x, y);` in `compute()` — has `add` call)
3. `debug_run` with `action: "continue"` to hit line 14
...
6. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 36 (`const result1 = compute(x, y);` in `main()`)
...
2. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true` — note session_id ("session-1")
3. `debug_sessions` — should return one session
4. `debug_launch` with `program: "/tmp/ts/debug_test.js"` again — should create "session-2"
...
1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 34
3. `debug_run` with `action: "continue"` to hit line 34
...
7. `/tmp/ts/debug_test.js` should appear in the sources list (the `.ts` file may also appear if source maps are resolved)
8. `debug_stop`
...
1. `debug_launch` with `program: "/tmp/ts/debug_test.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 4 (inside `add()`, called from `compute()`)
3. `debug_run` with `action: "continue"` to hit the breakpoint
...
1. `debug_launch` with `program: "/tmp/ts/debug_vars.js"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_vars.ts"` at line 32 (`x = modify(x, 3);` in `main()`)
3. `debug_run` with `action: "continue"` to hit line 32
...
3. `debug_breakpoint` with `action: "set"`, `file: "/tmp/ts/debug_test.ts"` at line 37 (`console.log(\`compute = ${result1}\`);`)
...
|---|-------------|--------|-------------|-------|
| 1 | Basic breakpoint + inspect (source-mapped) | PASS/FAIL | | |
