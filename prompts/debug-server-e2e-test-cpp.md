...
```json
{"name":"debug_launch","arguments":{"program":"/tmp/debug_test_cpp","stop_on_entry":true,"language":"cpp"}}
```

```json
{"name":"debug_breakpoint","arguments":{"session_id":"session-1","action":"set","file":"/tmp/debug_test.cpp","line":4}}
```
...
```bash
bash prompts/fixtures/cpp/setup.sh
```

This copies the source files from `prompts/fixtures/cpp/` to `/tmp/` and compiles them with
`c++ -g -O0`. Run the setup script at the start of every session to ensure clean state.
...
**Do NOT use sleep-based programs in this E2E.** Never launch `/tmp/debug_sleep_cpp`, never run `/usr/bin/sleep`, and never add waits via sleep commands.

### Programs

| Program | Source | Binary | Used by |
|---------|--------|--------|---------|
| A: Basic | `/tmp/debug_test.cpp` | `/tmp/debug_test_cpp` | Scenarios 1-11, 14-18, 20-28, 30-33 |
| B: Crasher | `/tmp/debug_crash.cpp` | `/tmp/debug_crash_cpp` | Scenario 19 |
| C: Multi-variable | `/tmp/debug_vars.cpp` | `/tmp/debug_vars_cpp` | Scenarios 12-13, 29 |

### Key Line References

**Program A** (`debug_test.cpp`):
...
- Line 16: `int final_ = sum + product;` (inside `compute()`)
...
- Line 37: `std::printf("compute = %d\n", result1);` (inside `main()`)

**Program B** (`debug_crash.cpp`):
- Line 6: `return a / b;` (divide by zero)
- Line 11: `std::printf("%d\n", *p);` (null deref, SIGSEGV)
- Line 15: `std::abort();` (SIGABRT)

**Program D** (`debug_vars.cpp`):
...
1. `debug_launch` with `program: "/tmp/debug_test_cpp"`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"`, `session_id`, `file: "/tmp/debug_test.cpp"`, `line: 4`
...
4. `debug_breakpoint` with `action: "set"` at line 16 (`int final_ = sum + product;` in `compute()`)
...
1. `debug_launch` with `program: "/tmp/debug_test_cpp"`
...
1. `debug_launch` with `/tmp/debug_test_cpp`, `stop_on_entry: true`
2. `debug_breakpoint` with `action: "set"` at line 4 (inside `add()`)
3. `debug_run` with `action: "continue"` to hit the breakpoint
...
Uses Program D (`/tmp/debug_vars_cpp`).

1. `debug_launch` with `/tmp/debug_vars_cpp`, `stop_on_entry: true`
...
**Note:** C++ uses the native DWARF engine, so all low-level features (registers, memory,
disassembly, symbol lookup, variable location) are fully supported.
...
1. `debug_launch` with `stop_on_entry: true`
2. `debug_breakpoint_locations` with `source: "/tmp/debug_test.cpp"`, `line: 3`, `end_line: 6`
   — should return valid breakpoint positions within `add()` (lines 4 and 5 at minimum)
3. `debug_breakpoint_locations` with `source: "/tmp/debug_test.cpp"`, `line: 12`
...
Uses Program B (`/tmp/debug_crash_cpp`).

1. `debug_launch` with `/tmp/debug_crash_cpp`, `args: ["null"]`, `stop_on_entry: true`
...
2. `debug_breakpoint` with `action: "set"` at line 14 (`int sum = add(x, y);` in `compute()`)
3. `debug_run` with `action: "continue"` to hit line 14
4. `debug_goto_targets` with `file: "/tmp/debug_test.cpp"`, `line: 16`
5. If targets are returned, `debug_run` with `action: "goto"`, `file: "/tmp/debug_test.cpp"`, `line: 16`
...
2. `debug_launch` with `/tmp/debug_test_cpp`, `stop_on_entry: true` — note session_id ("session-1")
3. `debug_sessions` — should return one session
4. `debug_launch` with `/tmp/debug_test_cpp` again — should create "session-2"
...
3. The main executable (`debug_test_cpp`) should appear in the modules list
4. Verify each module has identifying fields (name, path, or id)
5. `debug_loaded_sources` — should return source files
6. `/tmp/debug_test.cpp` should appear in the sources list
7. `debug_stop`
...
3. `debug_breakpoint` with `action: "set"` at line 37 (`std::printf("compute = %d\n", result1);`)
4. `debug_run` with `action: "continue"` to hit line 37
5. `debug_run` with `action: "step_over"` — `std::printf` should execute
...
8. `debug_registers` — if using DAP backend, should return NOT_SUPPORTED (-32001)
9. `debug_stop`
